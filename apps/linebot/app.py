...
def reply(intent,text,reply_token,id,disname):
   ...
   if intent == 'remove_subscribe':
      ...
      remove_subscriber(id)
      text_message = TextSendMessage(text="ดำเนินการลบ subscription ของคุณเรียบร้อย")
      line_bot_api.reply_message(reply_token, text_message)
   if intent == 'test':
   ...
   if intent == 'profile':
      ...
      if len(user_profile) == 0:
         text_message = TextSendMessage(text="เรายังไม่พบข้อมูลของคุณ%s กรุณาลงทะเบียนก่อน"%disname)
      else:
